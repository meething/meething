...
window.addEventListener('DOMContentLoaded', function () {
  ...
  rtcRoom.on("@open", ({ peers }) => {
    console.log(`Mediasoup - ${peers.length} peers in this room.`);
    h.getUserMedia()
      .then(async stream => {
        ...
        if (audioProducer == undefined) {
          ...
          audioProducer = await rtcRoom.sendAudio(audioTrack);
        }

        if (videoProducer == undefined) {
          const videoTrack = stream.getVideoTracks()[0];
          videoProducer = await rtcRoom.sendVideo(videoTrack);
        }
      });
  });
  rtcRoom.on("@consumer", async consumer => {
    const {
      id,
      appData: { peerId },
      track
    ...
    var el = document.getElementById(`${peerId}-video`);

    if (el) {
      el.srcObject.addTrack(track)
    } else {
      const stream = new MediaStream([track]);
      var el = h.addVideo(peerId);
      h.setVideoSrc(el, stream);
    }
  });
  rtcRoom.on("@peerClosed", ({ peerId }) => {
    h.closeVideo(peerId);
  });
  rtcRoom.on("@consumerClosed", ({ consumerId }) => {
    console.error("Consumer closed but we have a consumer id instead of peer id :(")
  });
...
function initRTC() {
  ...
  damSocket.on("postauth", function (auth) {
    ...
    damSocket.on('NewUserStart', function (data) {
      ...
      console.log("Start P2P new user start");
      ...
      if (data.description.type === "offer") {
        ...
        h.getUserMedia()
          .then(async stream => {
            ...
            const producer = await rtcRoom.sendVideo(track);
